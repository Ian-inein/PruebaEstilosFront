...
export async function loader() {
  const characters = await getInfo('https://rickandmortyapi.com/api/character');
...
export async function action({ request, params }) {
  ...
  const infoLocation = await getInfo(location);
  return { infoLocation };
}
...
export default function Index() {
  ...
  useEffect(() => {
    console.log(fetcher.data.infoLocation);
  }, [fetcher]);
  ...
  return (
    <div className=''>
      <h1 className=' '>Planetas Rick & Morty</h1>
      <div
        className=' sm:grid-cols-2 gap-6  p-5'
        style={{ fontFamily: 'system-ui, sans-serif', lineHeight: '1.8' }}
      >
        {characters.results.map((e) => (
          <div className='grid place-contet-center '>
            <button key={e.id} onClick={() => {}} className='group'>
              <div className='rounded-lg'>
                <img className=' transition-all' src={e.image} alt='' />
              </div>
              <div>
                ...
                <span className='flex '>
                  ...
                  <span className='font-normal  transition-all'>{e.location.name}</span>
                </span>
              </div>
            </button>
          </div>
